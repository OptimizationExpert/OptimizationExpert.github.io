...
description: "دوره جامع و پروژه‌محور مسیریابی وسایل نقلیه (VRP) و مسائل ترکیبی در Python با OR-Tools و CP-SAT؛ ۲۰ پروژهٔ عملی و حدود ۱۰ ساعت ویدیو: از حداکثر جریان، تخصیص، کوله‌پشتی و زمان‌بندی تا TSP، CVRP، VRPTW، مسیریابی چندانباره، تحویل تقسیم‌شده، مسیریابی خودروی برقی و مسیریابی موجودی-دینامیک، همراه با کد کامل و داده‌های آماده."
keywords: "VRP, مسیریابی وسایل نقلیه, Vehicle Routing Problem, TSP, CVRP, VRPTW, multi-depot, split delivery, EV routing, inventory routing, OR-Tools, CP-SAT, AddCircuit, بهینه سازی ترکیبی, Python, پایتون, لجستیک, حمل و نقل, مسیریابی, زنجیره تامین"
...
یک مسئلهٔ **ارضای قید (constraint satisfaction)** خالص: در جدول n×n اعداد ۱ تا n² را طوری بچینید که مجموع هر سطر، هر ستون و دو قطر برابر «عدد جادویی» شود. متغیر تصمیم، مقدار عددی هر خانه است و قید کلیدی، **`AddAllDifferent`** (همهٔ خانه‌ها متمایز) در کنار قیود تساوی مجموع‌هاست. این پروژه نشان می‌دهد CP-SAT برای مسائلی که «هدف» ندارند و فقط دنبال یک جواب شدنی‌اند چقدر قوی است.
...
> **نکتهٔ کلیدی —** همان الگوی **خطی‌سازی AND** دوباره به کار می‌آید؛ تسلط بر این الگو در سراسر دوره حیاتی است.
...
> **نکتهٔ کلیدی —** قید **پیش‌نیاز** (نیم‌سالِ c۱ پیش از نیم‌سالِ c۲) نمونهٔ تمیزی از مدل‌کردن ترتیب/تقدم است که در زمان‌بندی همه‌جا لازم می‌شود.

> **نکتهٔ کلیدی —** هدف **min-max** برای توازن بار درسی نشان می‌دهد چطور «متوازن‌بودن» را به یک هدف قابل‌بهینه‌سازی تبدیل کنیم.
...
> **نکتهٔ کلیدی —** **شکستن تقارن** (مثلاً بارِ خودرو c ≤ بارِ خودرو c+1) جواب‌های تکراری و هم‌ارز را حذف می‌کند و زمان حل را به‌شکل چشمگیری کوتاه می‌کند.
...
<script type="application/ld+json">
{
  ...
  "teaches": [
    "مسائل ترکیبی و مدل‌سازی با OR-Tools و CP-SAT",
    "حداکثر جریان، تخصیص، کوله‌پشتی و بسته‌بندی",
    "زمان‌بندی تولید، مربع جادویی و بزرگ‌ترین خوشهٔ گراف",
    "فروشندهٔ دوره‌گرد (TSP) با AddCircuit و هرس یال",
    "مسیریابی ظرفیت‌دار چندخودرویی (CVRP)",
    "پنجرهٔ زمانی (VRPTW) با زمان سرویس و انتظار",
    "مسیریابی چندانباره (Multi-Depot VRP)",
    "تحویل تقسیم‌شده (Split Delivery VRP)",
    "مسیریابی خودروی برقی با باتری و ایستگاه شارژ (EV Routing)",
    "ترابری/انتقال بار (Transshipment)",
    "مسیریابی موجودی و دینامیک (Inventory Routing)"
  ],
  "provider": {
