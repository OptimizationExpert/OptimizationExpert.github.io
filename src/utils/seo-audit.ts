...
async function buildValidPathSet(rawPages: RawPage[]): Promise<Set<string>> {
    ...
    for (const key of Object.keys(modules)) {
        ...
        if (rel.endsWith('/index')) {
            set.add(`/${rel.slice(0, -'/index'.length)}/`);
            continue;
        }
...
async function computeSeoReport(): Promise<SeoPage[]> {
    ...
    const pages: SeoPage[] = rawPages.map(p => {
        ...
        if (p.title.length < t.titleMin) issues.push({
            severity: 'warning',
            code: 'title-short',
            message: `عنوان کوتاه است (${p.title.length} نویسه؛ پیشنهاد: ${t.titleMin}-${t.titleMax})`
        });
        if (p.title.length > t.titleMax) issues.push({
            severity: 'warning',
            code: 'title-long',
            message: `عنوان بلند است (${p.title.length} نویسه) و ممکن است در نتایج گوگل قطع شود`
        });
        if (p.description.length < t.descriptionMin) issues.push({
            severity: 'error',
            code: 'desc-short',
            message: `توضیحات کوتاه است (${p.description.length} نویسه؛ پیشنهاد: ${t.descriptionMin}-${t.descriptionMax})`
        });
        if (p.description.length > t.descriptionMax) issues.push({
            severity: 'warning',
            code: 'desc-long',
            message: `توضیحات بلند است (${p.description.length} نویسه) و ممکن است قطع شود`
        });
        if (!p.hasImage) issues.push({
            severity: 'warning',
            code: 'no-image',
            message: 'بدون تصویر شاخص (برای اشتراک‌گذاری و نتایج جستجو مهم است)'
        });
        if (p.hasImage && !p.hasImageAlt) issues.push({
            severity: 'error',
            code: 'no-alt',
            message: 'تصویر شاخص متن جایگزین (alt) ندارد'
        });
        if (p.tags.length === 0) issues.push({
            severity: 'warning',
            code: 'no-tags',
            message: 'هیچ تگی ندارد (شانس دیده‌شدن از طریق صفحات تگ را از دست می‌دهد)'
        });
        if (p.wordCount < t.thinContentWords) issues.push({
            severity: 'error',
            code: 'thin-content',
            message: `محتوای متنی کم است (~${p.wordCount} کلمه؛ پیشنهاد حداقل ${t.thinContentWords})`
        });
        if (p.wordCount >= t.longContentForHeadings && p.headingCount === 0) issues.push({
            severity: 'warning',
            code: 'no-headings',
            message: 'متن طولانی است ولی زیرعنوان (## یا ###) ندارد؛ خوانایی و ساختار سئو ضعیف می‌شود'
        });
        if (p.wordCount >= t.thinContentWords && p.internalLinks.length === 0) issues.push({
            severity: 'warning',
            code: 'no-internal-links',
            message: 'هیچ لینک داخلی به صفحات دیگر سایت ندارد'
        });
        if (p.hasBodyH1) issues.push({
            severity: 'error',
            code: 'h1-conflict',
            message: 'متن یک تیتر سطح ۱ (# ...) دارد که با عنوان اصلی صفحه تداخل ایجاد می‌کند؛ در بدنه فقط از ## یا ### استفاده کنید'
        });
        if (p.longestParagraphWords > t.maxParagraphWords) issues.push({
            severity: 'warning',
            code: 'long-paragraph',
            message: `طولانی‌ترین پاراگراف حدود ${p.longestParagraphWords} کلمه است؛ برای خوانایی بهتر آن را به چند پاراگراف کوتاه‌تر بشکنید`
        });
        if (p.avgSentenceLength > t.maxAvgSentenceWords) issues.push({
            severity: 'warning',
            code: 'long-sentences',
            message: `میانگین طول جملات حدود ${p.avgSentenceLength} کلمه است؛ جملات کوتاه‌تر خوانایی را بهتر می‌کند`
        });
        if (p.wordCount >= t.longContentForImages && p.bodyImageCount === 0) issues.push({
            severity: 'warning',
            code: 'no-body-images',
            message: 'متن طولانی است ولی هیچ تصویری داخل بدنه ندارد؛ تصویر یا نمودار میانی می‌تواند خوانایی و زمان ماندگاری کاربر را بهبود دهد'
        });
        ...
        if (brokenLinks.length > 0) {
            ...
            issues.push({
                severity: 'error',
                code: 'broken-link',
                message: `${brokenLinks.length} لینک داخلی شکسته: ${sample}${brokenLinks.length > 3 ? ' و...' : ''}`
            });
        }

        const titleKey = p.title.trim().toLocaleLowerCase('fa-IR');
        if (titleKey && (titleCounts.get(titleKey) ?? 0) > 1) issues.push({
            severity: 'error',
            code: 'dup-title',
            message: 'این عنوان دقیقاً در یک صفحه‌ی دیگر هم استفاده شده (محتوای تکراری)'
        });
        const descKey = p.description.trim().toLocaleLowerCase('fa-IR');
        if (descKey && (descCounts.get(descKey) ?? 0) > 1) issues.push({
            severity: 'error',
            code: 'dup-desc',
            message: 'این توضیحات دقیقاً در یک صفحه‌ی دیگر هم استفاده شده (محتوای تکراری)'
        });
...
export function summarizeIssues(pages: SeoPage[]): SeoIssueSummary[] {
    ...
    for (const page of pages) {
        for (const issue of page.issues) {
            ...
            else byCode.set(issue.code, {
                code: issue.code,
                message: issue.message.replace(/\(.*?\)/g, '').replace(/:.*$/, '').trim(),
